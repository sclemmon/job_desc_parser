...
{{
  ...
  "gaps": ["skill the job wants but candidate does not have", ...],
...
- gaps should list skills from the job that have NO reasonable match in the candidate's inventory.
  IMPORTANT: Before flagging something as a gap, check whether it is a compound skill (e.g.
  "familiarity with experimentation in regulated industries" combines experimentation + regulated
  industries). Decompose it into its individual components and check each one against the inventory
  separately. Only flag it as a gap if one or more of the individual components are missing.
  If all components are covered, it is a match — do not list it as a gap.
